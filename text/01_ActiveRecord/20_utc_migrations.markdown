...
Prima dell'introduzione delle **timestamped migrations**, ogni nuova migration creata ha un numero progressivo nel suo nome. Se due migrations vengono generate da due sviluppatori e non vengono notificate immediatamente (nel repository comune), può succedere di avere due migrazioni differenti con stesso numero progressivo. A questo punto il vostro schema_info non è aggiornato e avrete un'inconsistenza nella vostra applicazione.
...
Se avete utilizzato Git allore potreste aver avuto problemi più pesanti, in quanto il vostro team probabilmente aveva una coppia di branches e delle **migrations** inconsistenti in ognuna di esse. Avrete avuto seri problemi di conflitti durante il merging dei branches.

Per risolvere questo grave problema il core team ha cambiato il modo con cui le **migrations** lavorano in Rails. Anziché anteporre ad ogni nome di file delle migrazioni un numero progressivo (corrispondente al valore corrente di schema_count), adesso si utilizza una stringa basata sul tempo **UTC** e corrispondente al formato  YYYYMMDDHHMMSS.

Inoltre, è stata introdotta una nova tabella chiamata **schema_migrations** che tiene traccia di quali **migrations** sono già state eseguite. In questo modo, se chiunque crea una **migration** con un numero "più piccolo", Rails esegue un **rollback** delle migration fino alla versione precedente e quindi riesegue le migrations fino alla versione corrente.
